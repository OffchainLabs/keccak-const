//! Tests converted from https://github.com/emn178/js-sha3/blob/master/tests/test-kmac.js

use keccak_const::KMac128;
use keccak_const::KMac256;

#[test]
fn kmac128_empty_customization_short_input() {
    let key: Vec<u8> = (0x40..=0x5f).collect();
    let output = KMac128::new(&key, b"")
...
#[test]
fn kmac128_tag_customization_short_input() {
    let key: Vec<u8> = (0x40..=0x5f).collect();
    let output = KMac128::new(&key, b"My Tagged Application")
        .update(&[0x00, 0x01, 0x02, 0x03])
        .finalize();
...
#[test]
fn kmac128_tag_customization_long_input() {
    let key: Vec<u8> = (0x40..=0x5f).collect();
...
#[test]
fn kmac256_tag_customization_short_input() {
    let key: Vec<u8> = (0x40..=0x5f).collect();
    let output = KMac256::new(&key, b"My Tagged Application")
        .update(&[0x00, 0x01, 0x02, 0x03])
        .finalize();
...
#[test]
fn kmac256_empty_customization_long_input() {
    let key: Vec<u8> = (0x40..=0x5f).collect();
    let input: Vec<u8> = (0x00..=0xC7).collect();

    let output = KMac256::new(&key, b"").update(&input).finalize();
...
#[test]
fn kmac256_tag_customization_long_input() {
    let key: Vec<u8> = (0x40..=0x5f).collect();
    let input: Vec<u8> = (0x00..=0xC7).collect();
...
#[test]
fn kmac128_no_key_no_input() {
    let tag: &[u8] = &[
        8, 79, 237, 8, 185, 120, 175, 77, 125, 25, 106, 116, 70, 168, 107, 88, 0, 158, 99, 107, 97,
        29, 177, 98, 17, 182, 90, 154, 173, 255, 41, 197, 8, 79, 237, 8, 185, 120, 175, 77, 125,
        25, 106, 116, 70, 168, 107, 88, 0, 158, 99, 107, 97, 29, 177, 98, 17, 182, 90, 154, 173,
        255, 41, 197, 8, 79, 237, 8, 185, 120, 175, 77, 125, 25, 106, 116, 70, 168, 107, 88, 0,
        158, 99, 107, 97, 29, 177, 98, 17, 182, 90, 154, 173, 255, 41, 197, 8, 79, 237, 8, 185,
        120, 175, 77, 125, 25, 106, 116, 70, 168, 107, 88, 0, 158, 99, 107, 97, 29, 177, 98, 17,
        182, 90, 154, 173,
    ];
    let output = KMac256::new(b"", &tag).finalize();

    assert_eq!(
        [
            0x03, 0x18, 0x01, 0xb0, 0xb5, 0x0e, 0xbe, 0xef, 0x77, 0x2f, 0xbe, 0x7a, 0x27, 0x9b,
            0xc1, 0x44
        ],
        output
    );
